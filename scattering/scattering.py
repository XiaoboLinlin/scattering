...
def structure_factor_pair(trj, trj1, pair, Q_range=(0.5, 50), n_points=1000, framewise_rdf=False, method='fz'):
    rho = np.mean(trj.n_atoms / trj.unitcell_volumes)
    L = np.min(trj.unitcell_lengths)
    ...
    for i, q in enumerate(Q):
        ...
        for (elem1, elem2) in it.product(elements_i,elements_j):
            ...
            try:
                ...
                N_i = len(top1.select('element {}'.format(atomtype1)))
                N_j = len(top1.select('element {}'.format(atomtype2)))

                N_i_1 = len(top.select('element {}'.format(e1)))
                N_i_2 = len(top.select('element {}'.format(e2)))
            ...
            if method == 'al':
                ...
                pre_factor = 4 * np.pi * rho
                partial_sq = (integral*pre_factor) * scale
